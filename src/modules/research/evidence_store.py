...
def load_full_ledger(db: Session) -> EvidenceLedger:
    """Load the complete durable evidence ledger in deterministic order."""
    rows = (
        db.query(ResearchEvidenceRecord)
        .order_by(
            ResearchEvidenceRecord.recorded_at.asc(),
    ...
    sources = (
        db.query(ResearchSourceRecord)
        .filter(ResearchSourceRecord.source_id.in_(source_ids))
        .order_by(ResearchSourceRecord.source_id.asc())
        .all()
    )
    for row in sources:
        ledger.register_source(
